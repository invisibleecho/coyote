...
var Validator = /** @class */ (function () {
    ...
    Validator.prototype.walk = function (data, schema, path) {
        ...
        schema.forEach(function (field) {
            ...
            try {
                _this.assert(fullName, field, data[name]);
                if (field.type === 'object' && field.children.length > 0) {
                    _this.walk(data[field.name] || {}, field.children, fullName + '.');
                }
            }
            catch (error) {
        ...
        unknownKeys.forEach(function (key) { return _this.errors.push({ name: path + key, message: "Field '".concat(path + key, "' is not defined within the provided schema.") }); });
    };
    /**
     ...
     * @throws Error
     */
    Validator.prototype.assert = function (fullName, field, value) {
        var _a;
        var type = field.type;
        ...
        switch (type) {
            ...
            case 'object':
                ...
                if (isOfValidType && type !== 'object')
                    this.decorated(fullName, field, value);
                break;
            case 'string[]':
            case 'number[]':
            case 'boolean[]':
        ...
        if (typeof passedType === 'undefined' && !field.optional) {
            throw new Error("Field ".concat(fullName, " (optional: ").concat(field.optional, ") was not passed. Consider marking it optional."));
        }
        if (!isOfValidType && (!field.optional || typeof passedType !== 'undefined')) {
            throw new Error("Field ".concat(fullName, " (optional: ").concat(field.optional, ", type: ").concat(passedType.toLowerCase(), ") does not match the defined type ").concat(type, "."));
        }
    };
    /**
     ...
     * @throws Error
     */
    Validator.prototype.decorated = function (fullName, field, value) {
        field.decorators.forEach(function (decorator) {
            ...
            if (!asserted) {
                throw new Error("Field ".concat(fullName, " (optional: ").concat(field.optional, ", decorator: ").concat(decorator.name, ") could not be validated."));
            }
        });
    };
    return Validator;
